import { useEffect, useRef, useState } from 'react'; import { useRouter } from 'next/router';
import NavLogo from '@/components/NavLogo'; import { useScanStore, makeDemoData, type ScanData } from '@/lib/state';
...
(async()=>{
  ...
  const timer=setInterval(()=>setIdx(i=>(i+1)%order.length),600);
  ...
  if(!data) data=makeDemoData(email);
